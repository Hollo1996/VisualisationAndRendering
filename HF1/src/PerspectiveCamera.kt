...
class PerspectiveCamera(vararg programs : Program) : UniformProvider("camera") {
  ...
  val position = Vec3(0.0f, 0.0f, 1.0f)
  ...
  init {
    update()
    register("rayDirMat", rayDirMatrix)
    register("eyePos", position)
  ...
  fun move(dt : Float, keysPressed : Set<String>) { 
    ...
    if("K" in keysPressed) {
      level.x = (level.x + 0.1f * (speed * dt)).coerceIn(0.0f,1.0f)
    ...
    ahead = (rotationMatrix*Vec3(0.0f, 0.0f, -1.0f).xyz0 ).xyz.normalize()
    right = (rotationMatrix*Vec3(1.0f, 0.0f, 0.0f).xyz0 ).xyz.normalize()
    up = (rotationMatrix*Vec3(0.0f, 1.0f, 0.0f).xyz0 ).xyz.normalize()
    update()
